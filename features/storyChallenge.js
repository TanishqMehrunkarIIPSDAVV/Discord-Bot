...
const handleChallengeCommand = async (message) => {
  ...
  if (sub === "help") {
    await message.reply([
      ...
      "This command only works in the dedicated IQ challenge channel set by storyChallengeChannelId in config.json.",
    ].join("\n")).catch(() => {});
    return;
  }
  ...
  if (!storyChannel) {
    await message.reply(
      "I could not resolve the dedicated IQ challenge channel. Set storyChallengeChannelId in config.json."
    ).catch(() => {});
    return;
  }
  ...
  const challengeEmbed = new EmbedBuilder()
    ...
    .setDescription([
      ...
      `Challenge channel: ${storyChannel}`,
