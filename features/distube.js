...
const distubeFunc=()=>
{
                ...
                else
                {
                    if(command==="play")
                    {
                        ...
                        distube.play(channel, url, {
                            ...
                            console.error(err);
                            if (err.name === 'DisTubeError' && err.message.includes('CANNOT_RESOLVE_SONG')) {
                                message.channel.send("Sorry, I couldn't resolve the song URL or search term.");
                            } else {
    ...
    distube
        ...
        .on('error', (e,textChannel) => {
            console.log(e);
            textChannel.send(
                `An error encountered: ${e.message}`,
            );
        })
        ...
        .on('searchInvalidAnswer', (message) => message.channel.send('Invalid selection.'));
