...
const PREFIX = "ct";
const POST_COMMAND = "postboymod";

const APPLY_BUTTON_ID = "boy_mod_apply";
const APPLICATION_MODAL_ID = "boy_mod_application_modal";

// Set your user ID here or via BOY_MOD_REVIEWER_ID env var.
const REVIEWER_USER_ID = (
    process.env.BOY_MOD_REVIEWER_ID ||
    process.env.GIRL_MOD_REVIEWER_ID ||
    "779206329813696522"
).trim();
...
function formatApplicationEmbed({ applicant, userId, username, hours, experience }) {
    return new EmbedBuilder()
        .setColor("#4d8dff")
        .setTitle("New Boy Mod Application")
        .addFields(
...
const boyModApplication = () => {
    client.on(Events.MessageCreate, async (message) => {
        ...
        const displayText = announcementText.length
            ? announcementText
            : "Applications for **Boy Mod** are now open. Click **Apply Now** and fill the form.";
        ...
        await message.reply("Boy mod application post sent.");
    });
